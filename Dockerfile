...
#copy entrypoint.sh
COPY ./entrypoint.sh /usr/src/ecommerceSite/entrypoint.sh
#RUN chmod a+x  /usr/src/ecommerceSite/entrypoint.sh   
RUN ["chmod", "755", "/usr/src/ecommerceSite/entrypoint.sh"]
...
ENTRYPOINT ["/usr/src/ecommerceSite/entrypoint.sh" ]

# set working directory | ceci est en fonction de l'environnement (Donc optionel)
#RUN mkdir -p /usr/src/ecommerceSite
#WORKDIR /usr/src/ecommerceSite